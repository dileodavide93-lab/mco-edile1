...
export const RecentWork: React.FC = () => {
  return (
    <section className="py-32 bg-zinc-950 text-white overflow-hidden">
      <div className="px-6 max-w-7xl mx-auto mb-16 flex flex-col md:flex-row justify-between items-end gap-8">
        <div>
          <h2 className="text-5xl md:text-7xl font-bold tracking-tighter mb-4">Lavori recenti</h2>
          <p className="text-red-500 text-xl font-medium">Qualità ed estetica d'avanguardia.</p>
        </div>
        <button className="px-8 py-4 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 hover:scale-105 transition-all">Vedi portfolio</button>
      </div>

      <div className="marquee-container">
        <div className="marquee-content flex gap-8">
          {[...images, ...images].map((src, i) => (
            <div key={i} className="flex-none w-[400px] h-[500px] rounded-[2rem] overflow-hidden border-4 border-zinc-800 hover:border-red-600 group relative transition-colors">
              ...
              <div className="absolute inset-0 bg-red-600/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                 <span className="px-6 py-2 bg-white text-red-600 font-bold rounded-full shadow-xl">Vedi progetto</span>
              </div>
            </div>
          ))}
        </div>
